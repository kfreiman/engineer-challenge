...
- Логика разных контекстов (например, `billing`, `profile`) должна иметь возможность взаимодействовать друг с другом.
...
Разделение на контексты существует на уровне кодовой базы приложения (`internal/...`). Однако, каждый контекст **полностью изолирован** и, архитектурно, способен иметь собственный сервер и точку входа. Вся логика деплоя, инициализации инфраструктуры и поднятия серверов вынесена исключительно в слой `cmd/` (wireup-уровень). В будущем, для масштабирования, достаточно будет добавить новые папки в `cmd/` (например, `cmd/billing`, `cmd/profile`), чтобы развернуть контексты как независимые сервисы, не меняя код в `internal/`.
...
graph TD
    subgraph "Docker Container / OS Process"
        ...
        BFF["BFF / API Gateway (GraphQL)"]
        Bus(("Watermill In-Memory Event Bus <br/> (В будущем NATS/Kafka)"))
        subgraph Context_Profile ["Bounded Context: Profile"]
            AppOrd["App Layer"]
            RepoOrd[/"Domain DB"/]
            AppOrd --> RepoOrd
        end
        ...
        Main -->|1. Инициализирует репозитории и адаптеры| Context_Profile
...
- **Готовность к масштабированию:** Логика деплоя (wireup) ограничена директорией `cmd/`. При необходимости масштабирования достаточно создать `cmd/profile/main.go` и заменить локальные адаптеры на сетевые gRPC/NATS клиенты. Код в `internal/...` останется нетронутым.
