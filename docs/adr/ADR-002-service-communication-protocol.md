...
- **Безсетевое (In-process) взаимодействие:** Допустимо для вызовов между доменными сервисами (Bounded Contexts) внутри одного приложения/бинарного файла (в соответствии с [ADR-005](./ADR-005-go-ddd-lite-structure.md)). Это исключает сетевой overhead, но требует строгого соблюдения интерфейсов для сохранения возможности выноса в отдельный микросервис.
