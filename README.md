...
### Цели (Goals)
- **Архитектурная зрелость**: Демонстрация применения DDD (Bounded Contexts) и CQRS для создания поддерживаемой системы.
- **Безопасность "по умолчанию"**: Реализация надежных флоу аутентификации (регистрация, вход, восстановление) с использованием проверенных инструментов.
- **Типизированный контракт**: Использование GraphQL для четкого взаимодействия между фронтендом и бэкендом.
- **Наблюдаемость**: Обеспечение прозрачности работы системы через логи, метрики и трейсинг (Observability).
- **Воспроизводимость**: Легкий запуск всей инфраструктуры одной командой через Docker Compose.

### Не-цели (Non-goals)
- **Изобретение IAM**: Мы не пишем свою реализацию хранения паролей и сессий с нуля, доверяя это Ory Kratos.
- **Микросервисы ради микросервисов**: Мы используем модульный монолит, чтобы избежать проблем распределенных систем, сохраняя возможность разделения в будущем.
- **Полноценный продуктовый функционал**: Фокус сделан именно на модуле аутентификации; другие домены (Billing, Profile) представлены как архитектурные заготовки.
- **Production-ready инфраструктура**: Настройка Kubernetes, Vault и Managed DB вынесена в "следующие шаги" и не является целью данного этапа.
...
## Что нужно сделать для выхода в production

Текущая реализация — это надежный фундамент. Для полноценного запуска в production необходимо:

### 1. Инфраструктура и Данные

- **Persistence**: Развертывание отказоустойчивых кластеров PostgreSQL для бизнес-доменов (Profile, Billing) с настроенным бэкапированием и репликацией.
- **Migrations**: Внедрение инструмента миграций в CI/CD пайплайн.
- **Orchestration**: Переход с Docker Compose на **Kubernetes (Helm)** для обеспечения High Availability и горизонтального масштабирования.

### 2. Безопасность и Секреты

- **Vault**: Перенос конфиденциальных данных и ключей из `.env` в **HashiCorp Vault** или облачные Secret Managers.
- **Edge Security**: Настройка TLS на Traefik с использованием Let's Encrypt/Cloudflare или корпоративных сертификатов.

### 3. Настройка Observability

- **Observability**: Реализовать сбор логов, метрик и трассировок ([ADR-008](docs/adr/ADR-008-observability-stack.md))
- **Alerting**: Настройка алертинга в Grafana на основе метрик Prometheus и ошибок в Loki.

### 4. Операбельность и Коммуникации

- **Real SMTP**: Замена Mailpit на промышленный провайдер (SendGrid, AWS SES) с настройкой DKIM/SPF/DMARC.
- **Observability**: Настройка алертинга в Grafana на основе метрик Prometheus и ошибок в Loki ([ADR-008](docs/adr/ADR-008-observability-stack.md)).
- **CI/CD**: Полноценный пайплайн с автоматическим прогоном E2E тестов в ephemeral-окружениях перед деплоем.
